...
for line in massesFile:
  ...
  massDir = datacards_dir + "/" + str(line)
  ...
  scriptFile.write('cp '+massDir+'/model*.root $WORKDIR\n')
  scriptFile.write('cd $WORKDIR\n')
  scriptFile.write('combine model.root -M MarkovChainMC -m ' + str(line) + '-H ProfileLikelihood -U >&! log.txt')

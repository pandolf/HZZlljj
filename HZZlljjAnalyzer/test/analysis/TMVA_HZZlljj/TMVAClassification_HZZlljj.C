...
void TMVAClassification( int mass=400, int nbtags=-1 )
{
 ...
 //}
   ...
   if( nbtags>=0 )
     sprintf( outfileName, "TMVAKIN_%d_btag%d.root", mass, nbtags );
   else
     sprintf( outfileName, "TMVAKIN_%d.root", mass );
...
///factory->AddVariable("absEtaLept2",  "Sublead Lepton |eta|", "", 'F');
 ...
 //if( nbtags != 2 ) factory->AddVariable( "ptJet2",  "Sublead Jet p_{T}", "GeV/c", 'F');
 //factory->AddVariable( "ptJet1_preKin",  "Lead Jet p_{T}", "GeV/c", 'F');
///factory->AddVariable("absEtaJet1", "Lead Jet |eta|", "", 'F');
 //factory->AddVariable( "ptJet2_preKin",  "Sublead Jet p_{T}", "GeV/c", 'F');
///factory->AddVariable("absEtaJet2", "Sublead Jet |eta|", "", 'F');
 //factory->AddVariable( "ptJetRecoil",  "Recoil Jet p_{T}", "GeV/c", 'F');
   ...
   //if( nbtags<=0 )  factory->AddVariable( "QGLikelihoodJet1Jet2",  "QG LD Product", "", 'F');
...
///factory->AddVariable("deltaR_recoil_Higgs", "#DeltaR (Recoil - Higgs)", "", 'F');
 ...
 //else
   factory->AddVariable("deltaRjj_preKin",  "#DeltaR (Jet-Jet)", "", 'F');
...
///factory->AddVariable("absDeltaPhiZZ",  "Z-Z Delta #phi", "rad", 'F');
 ...
 //factory->AddVariable("pfMet",  "PFMet", "GeV", 'F');
  ...
  //}
   ...
   if( nbtags<=0 )  mycuts += "QGLikelihoodJet1Jet2>0.1";

   if( mass>=500 ) {
       if( nbtags<=0 ) mycuts += "ptZll>100.";
       mycuts += "ptJet1>50.";
       mycuts += "deltaRjj_preKin<2.";
   } else if( mass>=400 ) {
       if( nbtags<=0 ) mycuts += "ptZll>70.";
       mycuts += "ptJet1>40.";
       mycuts += "deltaRjj_preKin<2.";
   } else if(mass>=300) {       
       if( nbtags<=0 ) mycuts += "ptZll>50.";
   } else {
       mycuts  = "ptZll>20.";
    }
 ...
 //                                     "nTrain_Signal=0:nTrain_Background=0:SplitMode=Random:NormMode=NumEvents:!V" );
   ...
   if (Use["Cuts"]) {
      ...
      bookConditions += ":VarProp[0]=FMax"; //ptZll
      bookConditions += ":VarProp[1]=FMax"; //ptJet1
      //bookConditions += ":VarProp[2]=FMax"; //ptJet2
      bookConditions += ":VarProp[2]=FMin"; //deltaRjj_prekin
      bookConditions += ":VarProp[3]=NotEnforced"; //ZZ mass
    //  bookConditions += ":VarProp[1]=FMax"; //ptJet1
    //  bookConditions += ":VarProp[2]=FMax"; //ptJet2
     // bookConditions += ":VarProp[3]=FMax"; //QG Likeli Product
     // bookConditions += ":VarProp[4]=FMax"; //QG Likeli jet1jet2recoil
     // bookConditions += ":VarProp[4]=FMax"; //QG Likeli JetRecoil
    //  bookConditions += ":VarProp[3]=FMin"; //deltaRjj
     // bookConditions += ":VarProp[5]=NotEnforced"; //ZZ mass
   ...
   if (Use["Cuts"] || Use["CutsGA"]) {
     ...
     for( unsigned iEff=1; iEff<11; ++iEff ) {
       ...
       if( nbtags>=0 )
         sprintf( cutsFileName, "CUTSKIN/cutsKIN%s_btag%d_%d_Seff%d.txt", cutsType.c_str(), nbtags, mass, 10*iEff );
       else
         sprintf( cutsFileName, "CUTSKIN/cutsKIN%s_%d_Seff%d.txt", cutsType.c_str(), mass, 10*iEff );
     ...
     //  cuts->GetCuts((float)iEff/10., cutsMin, cutsMax);
       ...
       ofs << "ptJet2 35. 1e+30" << std::endl;
       ofs << "mZjj 75. 105." << std::endl;
       ofs << "mZll 70. 110." << std::endl;
       if( nbtags<=0 ) ofs << "QGLikelihoodJet1Jet2 0.1 1e+30" << std::endl;

...
void TMVAClassification_helicityLD( int mass=400, int nbtags=-1 )
{
 ...
 //}
   ...
   if( nbtags>=0 )
     sprintf( outfileName, "TMVAHel_%d_btag%d.root", mass, nbtags );
   else
     sprintf( outfileName, "TMVAHel_%d.root", mass );
 ...
 //                                     "nTrain_Signal=0:nTrain_Background=0:SplitMode=Random:NormMode=NumEvents:!V" );
   ...
   if (Use["Cuts"]) {
      ...
      if( nbtags<=0 ) {
        ...
        bookConditions += ":VarProp[1]=FMax"; //QG Likeli Product
        bookConditions += ":VarProp[2]=NotEnforced"; //ZZ mass
      } else {
        bookConditions += ":VarProp[0]=FMax"; //helicityLD
   ...
   if (Use["Cuts"] || Use["CutsGA"]) {
     ...
     for( unsigned iEff=1; iEff<11; ++iEff ) {
       ...
       if( nbtags>=0 )
         sprintf( cutsFileName, "CUTSHEL/cuts%sHel_btag%d_%d_Seff%d.txt", cutsType.c_str(), nbtags, mass, 10*iEff );
       else
         sprintf( cutsFileName, "CUTSHEL/cuts%sHel_%d_Seff%d.txt", cutsType.c_str(), mass, 10*iEff );

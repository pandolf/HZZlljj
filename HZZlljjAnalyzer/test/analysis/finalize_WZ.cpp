...
int main( int argc, char* argv[] ) {
  ...
  if( argc!=3 && argc!=4 ) {
    std::cout << "USAGE: ./finalize_WZ [dataset] [selectionType] [leptType=\"ALL\"]" <<std::endl;
    return 13;
  }
  ...
  Ntp1Finalizer_WZ* nf = new Ntp1Finalizer_WZ( dataset, selectionType, leptType );
  ...
  } else if( dataset=="Mu_Dec22ReReco" ) {

    nf->addFile("Mu_Run2010A-Dec22ReReco_v1");
    nf->addFile("Mu_Run2010B-Dec22ReReco_v1");

  } else if( dataset=="EleMu_Dec22ReReco" ) {
